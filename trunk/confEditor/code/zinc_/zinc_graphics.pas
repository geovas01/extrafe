  ...
  if Started = True then
    ...
    begin
      if graph_selected = 'OpenGL' then
        render_path := Program_Path + 'media\emulators\arcade\zinc\config\opengl_renderer.cfg';
      if graph_selected = 'D3D' then
        render_path := Program_Path + 'media\emulators\arcade\zinc\config\d3d_renderer.cfg';
    end;
  if graph_selected = 'OpenGL' then
    begin
      if Started  = True then
...
//
      ...
      if Conf.rb9.Checked = True then
        begin
          graphicssave.SaveToFile(Program_Path + 'media\emulators\arcade\zinc\config\opengl_renderer.cfg');
          graphicssavedis.SaveToFile(Program_Path + 'media\emulators\arcade\zinc\config\d3d_renderer.cfg');
        end
      else
        begin
          graphicssavedis.SaveToFile(Program_Path + 'media\emulators\arcade\zinc\config\opengl_renderer.cfg');
          graphicssave.SaveToFile(Program_Path + 'media\emulators\arcade\zinc\config\d3d_renderer.cfg');
        end;
      graphicssave.Free;
